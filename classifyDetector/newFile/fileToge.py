...
if __name__ == "__main__":
    ...
    clf = LDA()
    pathTrain='/home/hao/桌面/lrpython/data/20fea/trainAll.txt'
    ...
    pathTest = '/home/hao/桌面/lrpython/data/20fea/test/'
    for root, dirs, files in os.walk(pathTest):
        for file in files:
            print file
            y, X, L = loadNormal202(pathTest+file)
            p = clf.predict(X)
            fileoutputpath='/home/hao/桌面/lrpython/longid/'+model+'/fold2/test/'
            ...
            for li in range(0,len(p)):
                if(str(p[li])=='1'):
                    fwrite.write(str(L[li]).replace('#',''))
                   # fwrite.write('\n')
            fwrite.close()


    # scores = scv.cross_val_score(clf, X, y, cv=5)
    pathTest = '/home/hao/桌面/lrpython/data/20fea/train/'
    for root, dirs, files in os.walk(pathTest):
        for file in files:
            print file
            y, X, L = loadNormal202(pathTest + file)
            ...
            fileoutputpath = '/home/hao/桌面/lrpython/longid/'+model+'/fold2/train/'
            if os.path.exists(fileoutputpath):
                print '',
            else:
                os.makedirs(fileoutputpath)
